...
public class URIResolver {
    ...
    public static class ContainerURIResolverNormal {
        ...
        static URI resolveURI(final Map<String, BagInfoField> map, final String containerKey,
                              final String resource, final int pathType) {
            final String hostname = getMapValue(map, ProfileOptions.REPOSITORY_HOSTNAME_KEY);
            Integer port = Integer.parseInt(getMapValue(map, ProfileOptions.PORT_KEY));
            if (port == 80) {
                port = -1;
            }
            final String appKey = getMapValue(map, ProfileOptions.REPOSITORY_PARTITION);
            final String restKey = getMapValue(map, ProfileOptions.RESOURCE_BASE_CONTEXT);
        ...
        public URIResolver build() throws URISyntaxException {
            final URI resolvedURI =
                    resolveURI(this.map, this.containerKey, this.resource, this.pathType);
