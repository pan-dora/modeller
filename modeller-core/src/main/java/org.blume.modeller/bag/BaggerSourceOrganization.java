...
import org.blume.modeller.Contact;
...
public class BaggerSourceOrganization implements Serializable{
  ...
  public BaggerSourceOrganization(BagInfoTxt bagInfoTxt) {
    ...
    if (bagInfoTxt.getContactName() != null && !bagInfoTxt.getContactName().trim().isEmpty()) {
      contact.setContactName(ProfileField.createProfileField(Contact.FIELD_CONTACT_NAME, bagInfoTxt.getContactName()));
    }
    else {
    ...
    if (bagInfoTxt.getContactPhone() != null && !bagInfoTxt.getContactPhone().trim().isEmpty()) {
      contact.setTelephone(ProfileField.createProfileField(Contact.FIELD_CONTACT_PHONE, bagInfoTxt.getContactPhone()));
    }
    else {
    ...
    if (bagInfoTxt.getContactEmail() != null && !bagInfoTxt.getContactEmail().trim().isEmpty()) {
      contact.setEmail(ProfileField.createProfileField(Contact.FIELD_CONTACT_EMAIL, bagInfoTxt.getContactEmail()));
    }
    else {
